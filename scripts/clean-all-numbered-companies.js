import { createClient } from '@supabase/supabase-js';
import dotenv from 'dotenv';

dotenv.config();

const supabase = createClient(
  process.env.VITE_SUPABASE_URL,
  process.env.VITE_SUPABASE_ANON_KEY
);

const stats = {
  found: 0,
  deleted: 0,
  errors: 0,
  claimed: 0
};

console.log('🗑️  NETTOYAGE COMPLET - TOUTES COMPAGNIES À NUMÉROS\n');
console.log('═══════════════════════════════════════════════════════════\n');
console.log('Cible: Toutes entreprises avec numéros + Canada/Quebec/Ontario Inc/Ltée\n');
console.log('═══════════════════════════════════════════════════════════\n');

console.log('🔍 Chargement des entreprises...\n');

// Charger toutes les entreprises
let allBusinesses = [];
let from = 0;
const pageSize = 1000;
let hasMore = true;

while (hasMore) {
  const { data, error } = await supabase
    .from('businesses')
    .select('id, name, city, data_source, is_claimed')
    .range(from, from + pageSize - 1);

  if (error || !data || data.length === 0) {
    hasMore = false;
  } else {
    allBusinesses = allBusinesses.concat(data);
    from += pageSize;
    hasMore = data.length === pageSize;

    if (from % 100000 === 0) {
      console.log(`   Chargé: ${from} entreprises...`);
    }
  }
}

console.log(`✅ ${allBusinesses.length} entreprises chargées\n`);

console.log('🔍 Filtrage des compagnies à numéros...\n');

// Patterns pour identifier les compagnies à numéros:
// 1. Numéros + CANADA/QUEBEC/ONTARIO + INC/LTÉE (n'importe quel chiffre au début)
// 2. Numéros avec tirets + CANADA/QUEBEC/ONTARIO + INC/LTÉE
// Exemples: "1041365 ONTARIO INC.", "9158-8459 QUEBEC INC", "123456 CANADA LTÉE"

const patterns = [
  // Pattern 1: Commence par des chiffres (avec ou sans espaces/tirets) + CANADA/QUEBEC/ONTARIO + INC/LTÉE
  /^\d+[\s-]*\d*[\s-]*\d*\s+(canada|quebec|ontario)\s+(inc\.?|ltée?\.?|limited|limitée|corporation|corp\.?)$/i,

  // Pattern 2: Seulement chiffres et tirets/espaces avant INC/LTÉE (pour capturer les formats comme "1234-5678 INC")
  /^[\d\s-]+\s+(inc\.?|ltée?\.?|limited|limitée|corporation|corp\.?)$/i,
];

const toDelete = allBusinesses.filter(b => {
  if (!b.name) return false;
  const name = b.name.trim();

  // Tester contre tous les patterns
  return patterns.some(pattern => pattern.test(name));
});

stats.found = toDelete.length;
console.log(`✅ Trouvé ${stats.found} compagnies à numéros\n`);

if (stats.found === 0) {
  console.log('🎉 Aucune entreprise à supprimer!');
  process.exit(0);
}

// Grouper par pattern pour les stats
const byType = {
  canada: 0,
  quebec: 0,
  ontario: 0,
  other: 0
};

toDelete.forEach(b => {
  if (!b.name) return;
  const name = b.name.toLowerCase();
  if (name.includes('canada')) byType.canada++;
  else if (name.includes('quebec')) byType.quebec++;
  else if (name.includes('ontario')) byType.ontario++;
  else byType.other++;
});

console.log('📊 Répartition par type:');
console.log(`   - CANADA INC/LTÉE: ${byType.canada}`);
console.log(`   - QUEBEC INC/LTÉE: ${byType.quebec}`);
console.log(`   - ONTARIO INC/LTÉE: ${byType.ontario}`);
console.log(`   - Autres: ${byType.other}`);
console.log();

// Montrer des exemples variés
console.log('📋 Exemples (premiers 100):');
toDelete.slice(0, 100).forEach(b => {
  const claimed = b.is_claimed ? '⚠️  RÉCLAMÉE' : 'Non réclamée';
  console.log(`   - "${b.name}" (${b.city || 'N/A'}) - ${claimed}`);
});
console.log();

// Compter les réclamées
stats.claimed = toDelete.filter(b => b.is_claimed).length;
if (stats.claimed > 0) {
  console.log(`⚠️  ATTENTION: ${stats.claimed} entreprises sont RÉCLAMÉES!`);
  console.log(`   Ces entreprises ne seront PAS supprimées.\n`);
}

// Filtrer pour exclure les réclamées
const toDeleteFiltered = toDelete.filter(b => !b.is_claimed);
console.log(`📊 ${toDeleteFiltered.length} entreprises non réclamées à supprimer\n`);

if (toDeleteFiltered.length === 0) {
  console.log('✅ Aucune entreprise non réclamée à supprimer!');
  process.exit(0);
}

console.log('⏸️  PAUSE: Vérifiez les exemples ci-dessus.');
console.log('   Assurez-vous que ce sont bien des entreprises à supprimer.\n');
console.log('   Suppression dans 10 secondes...\n');

// Pause de 10 secondes pour permettre d'annuler si besoin
await new Promise(resolve => setTimeout(resolve, 10000));

// Supprimer par batch
console.log('🗑️  Suppression en cours...\n');

const BATCH_SIZE = 100;
const TOTAL_BATCHES = Math.ceil(toDeleteFiltered.length / BATCH_SIZE);

for (let i = 0; i < toDeleteFiltered.length; i += BATCH_SIZE) {
  const batch = toDeleteFiltered.slice(i, i + BATCH_SIZE);
  const currentBatch = Math.floor(i / BATCH_SIZE) + 1;

  const promises = batch.map(business =>
    supabase
      .from('businesses')
      .delete()
      .eq('id', business.id)
      .then(({ error }) => {
        if (error) {
          stats.errors++;
          return false;
        }
        stats.deleted++;
        return true;
      })
  );

  await Promise.all(promises);

  console.log(`   🗑️  Batch ${currentBatch}/${TOTAL_BATCHES} - ${stats.deleted}/${toDeleteFiltered.length} supprimées - Erreurs: ${stats.errors}`);
}

console.log('\n✅ SUPPRESSION TERMINÉE!\n');
console.log('═══════════════════════════════════════════════════════════');
console.log('📊 STATISTIQUES FINALES:');
console.log('═══════════════════════════════════════════════════════════');
console.log(`Total trouvé: ${stats.found}`);
console.log(`  - CANADA INC/LTÉE: ${byType.canada}`);
console.log(`  - QUEBEC INC/LTÉE: ${byType.quebec}`);
console.log(`  - ONTARIO INC/LTÉE: ${byType.ontario}`);
console.log(`  - Autres: ${byType.other}`);
console.log();
console.log(`Entreprises réclamées (conservées): ${stats.claimed}`);
console.log(`Entreprises supprimées: ${stats.deleted}`);
console.log(`Erreurs: ${stats.errors}`);
console.log(`Taux de succès: ${toDeleteFiltered.length > 0 ? ((stats.deleted / toDeleteFiltered.length) * 100).toFixed(2) : 0}%`);
console.log('═══════════════════════════════════════════════════════════');

...
async function updateOnlyLevel2() {
  console.log('🔧 Mise à jour des parent_code pour LEVEL 2 uniquement...\n');

  console.log('='.repeat(70));

  const updateSQL = `
-- Mettre à jour UNIQUEMENT les codes de level 2
-- Pour qu'ils pointent vers les catégories principales (act_econ_main)
-- Level 3 garde ses parents level 2 actuels
...
SET parent_code = CASE
  ...
  ELSE parent_code
END
WHERE category_level = 2;  -- SEULEMENT level 2, pas level 3!
...
-- Vérifier que tous les level 2 pointent maintenant vers act_econ_main
...
`;
  ...
  console.log('\n✅ Résultat attendu:');
  console.log('   - 321 codes level 2 mis à jour → parent = code principal (0100, 0200, etc.)');
  console.log('   - 929 codes level 3 INCHANGÉS → parent = code level 2 (0110, 0120, etc.)');
  console.log('\n📊 Structure finale:');
  console.log('   act_econ_main (74 codes principaux: 0100, 0200, ...)');
  console.log('   └─ act_econ_codes level 2 (321 codes: 0110, 0120, ...)');
  console.log('      └─ act_econ_codes level 3 (929 codes: 0111, 0112, ...)');
}

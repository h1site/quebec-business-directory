...
async function syncMainCategorySlug() {
  console.log('🎯 SYNC MAIN_CATEGORY_SLUG FROM ACT_ECON\n');
  console.log('='.repeat(80));
  console.log('\nCe script va:');
  console.log('1. Assigner main_category_id via act_econ_to_main_category mapping');
  console.log('2. Remplir main_category_slug depuis main_categories.slug\n');
  console.log('='.repeat(80));
  ...
  if (mappingError) {
    console.error('❌ Erreur: La table act_econ_to_main_category n\'existe pas');
    console.error('   Veuillez d\'abord créer les mappings ACT_ECON → main_category');
    return;
  }
  ...
  if (needCategoryId > 0) {
    ...
    while (offset < needCategoryId) {
      ...
      // Group by act_econ main code (first 4 chars)
      const codeGroups = {};
      ...
      if (batchNumber % 10 === 0) {
        console.log(`   ⏳ Batch ${batchNumber}: ${totalUpdatedIds.toLocaleString()} mis à jour`);
      }
    }

    console.log(`\n✅ Étape 1 terminée: ${totalUpdatedIds.toLocaleString()} main_category_id assignés`);
  } else {
